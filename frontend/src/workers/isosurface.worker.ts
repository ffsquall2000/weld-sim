...
function interpolateVertex(
  ...
  const s1 = scalars[v1] ?? 0
  const s2 = scalars[v2] ?? 0
  const t = (threshold - s1) / (s2 - s1 + 1e-10)

  const p1x = positions[v1 * 3] ?? 0
  const p1y = positions[v1 * 3 + 1] ?? 0
  const p1z = positions[v1 * 3 + 2] ?? 0
  const p2x = positions[v2 * 3] ?? 0
  const p2y = positions[v2 * 3 + 1] ?? 0
  const p2z = positions[v2 * 3 + 2] ?? 0
...
function extractIsosurface(
  ...
  for (let t = 0; t < numTets; t++) {
    ...
    const tetIndices: number[] = [i0, i1, i2, i3]

    // Determine configuration
    let config = 0
    if ((scalars[i0] ?? 0) >= threshold) config |= 1
    if ((scalars[i1] ?? 0) >= threshold) config |= 2
    if ((scalars[i2] ?? 0) >= threshold) config |= 4
    if ((scalars[i3] ?? 0) >= threshold) config |= 8

    const triEdges = TRI_TABLE[config]
    if (!triEdges || triEdges.length === 0) continue
...
self.onmessage = (event: MessageEvent<IsosurfaceRequest>) => {
  ...
  if (type === 'extract') {
    ...
    ;(self as unknown as Worker).postMessage(response, [result.vertices.buffer, result.normals.buffer] as Transferable[])
  }
}
